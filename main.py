...
def subdivideImageActualTransformation(filename, dir_in, dir_out, d, pictureArray):
    ...
    #### MAIN Methode COPY
    smp = ShowMatrixPic(width=750, height=750, row=40, column=40, atuoTile=False)
 
    imgListOne = []

    import random

    for j in range(0, 1200,d):
        for i in range(0,1200,d):
            box = (j, i, j+d, i+d)
    ...
    # make curent_datetime readable with:
    current_datetime = datetime.strptime(current_datetime, "%Y%m%d_%H%M%S")

    pic_name = f"{input_pic.split('.')[0]}_{current_datetime}.jpg"

    #show pic directly 
    #cv2.imshow('CroppedImage', numpy_horizontal)
 

    cv2.imwrite(pic_name, numpy_horizontal, [cv2.IMWRITE_JPEG_QUALITY, 10])
...
input_pic = 'haus2.jpg'
source_folder = "photos_fs_plus_color"
def main():
    subdivideImageActualTransformation(input_pic, source_folder, "results", 30, makeDirectoryOfInputPictures(source_folder))
